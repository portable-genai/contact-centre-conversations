"""Platform-remote GuardrailPort: the Hrz1 Agent Guardrail Gateway client.

    POST <base>/v1/screen  {"text": str, "direction": "inbound"}
    -> {"verdict": "clean" | "blocked", "categories": [str], "detail": str}

Mkt6's platform family is the reference for the transport. Two rules this client keeps:
...
class PlatformGuardrailAdapter:
    """Screen one turn through the shared Hrz1 gateway."""

    def __init__(self, settings: Settings) -> None:
        self._settings = settings

    def screen(self, text: str, *, turn_index: int = 0) -> ScreenResult:
        base = require_base_url(self._settings.guardrail_url, what="guardrail_url (Hrz1)")
        ...
        if verdict not in {ScreenOutcome.CLEAN.value, ScreenOutcome.BLOCKED.value}:
            ...
            raise ValueError(f"Hrz1 returned an unrecognised verdict {verdict!r}")
