...
def test_the_demo_data_is_obviously_fictional() -> None:
    blob = " ".join(
        (
            *(text for text, _, _ in demo.COMPLIANT_TURNS),
            *(text for text, _, _ in demo.NON_COMPLIANT_TURNS),
            *(text for text, _ in demo.SELF_SERVICE_TURNS),
            demo.PII_TURN_TEXT,
            demo.CONSEQUENTIAL_TURN,
            demo.ACTOR,
            demo.TENANT,
        )
    )
    ...
    # An RFC 5737 documentation literal, never a routable address.
    assert "192.0.2." in blob
    for contact_id in (
        demo.COMPLIANT_CONTACT,
        demo.NON_COMPLIANT_CONTACT,
        demo.SELF_SERVICE_CONTACT,
    ):
        assert contact_id.startswith("demo-"), contact_id + " could be mistaken for a real id"


def test_the_demo_shows_both_modes_and_names_both_gates() -> None:
    """One repo, two separately gated modes: a demo that showed one would show half the design."""
    run = demo.DemoRun()
    run.run_to_end()
    state = run.state()
    opened = next(step for step in state["steps"] if step["key"] == "opened")
    assert opened["facts"]["modes"] == {"agent_assist": True, "self_service": True}
    audit = next(step for step in state["steps"] if step["key"] == "audit")
    assert sorted(audit["facts"]["modes"]) == ["agent_assist", "self_service"], (
        "the audit trail must carry records from both modes, tagged, or neither mode's "
        "promotion evidence can be separated from the other's"
    )
    checker = next(step for step in state["steps"] if step["key"] == "maker_checker")
    assert checker["facts"]["executor_calls"] == 0
...
def test_the_renderer_escapes_content_rather_than_interpolating_it() -> None:
    state = {
        ...
        "region": "r",
        "step_count": 1,
        "totals": {"turns": 0, "escalated": 0, "routed": 0, "chain_ok": True},
        "steps": [
            {
                "key": "k",
                "label": "l",
