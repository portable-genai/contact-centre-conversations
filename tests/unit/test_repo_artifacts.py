...
def test_the_lock_target_cannot_reintroduce_the_bare_uv_command() -> None:
    """The Makefile must not call uv directly, or the header goes again.

    This repository shipped without `scripts/lock.py` while still carrying the tests that check
    the header it restores, so its own documented `make lock` destroyed the header and turned its
    gate red. The defect was latent: the committed lockfiles had a header, and nothing ran the
    target until a fleet-wide refresh did. This is the guard that keeps it from returning.
    """
