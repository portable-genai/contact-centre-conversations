...
| Script | What it does |
|---|---|
| `demo.py` | The scripted arc, driving the REAL services over the `local` profile: bind the stack with both mode gates named, run an agent-assist contact through the whisper panel, let a disclosure window close unsatisfied and route it (rule R8), plant a national id and prove it never reaches the store or the record, run the self-service gate against an allowlisted ask, an out-of-scope one and a prompt injection, ask for a consequential action and count the ZERO executor calls, verify and export the mode-tagged audit trail, rewrite a record and detect it, then swap to the exit profile and watch every seam refuse. Writes the audit-view JSON. |
| `../eval/run_narrative_eval.py` | The judged half of the eval: grades each narrative against `config/quality-floors.toml` and checks the measured band equals the degradation table's expectation. Offline by default (`--judge deterministic`), so it runs inside `make gate`; `--judge local-model` sends the narratives to an endpoint you name. |
| `record_gemini_fixtures.py` | An AUTHORING step, never a gate step. Makes real managed-model calls once and writes `eval/datasets/gemini_replay.jsonl`, so `eval/run_eval.py --drafter replay-gemini` can score the rubrics against real model text with nothing reachable. Every reply is scanned for personal data, planted identifiers and length before any of it is written: one hit aborts the whole batch. |
| `render_evals_doc.py` | Regenerates the derived half of `docs/evals.md` from the rubrics, the scenarios and the quality floors. `--check` makes staleness a build failure rather than something a reader discovers, and `tests/unit/test_evals_doc.py` runs it. |
| `render_eval_report.py` | Renders the eval artifact from `eval/run_eval.py --emit` to ONE self-contained page: metric bars against their thresholds, rollups by vertical, market and scenario family, then every conversation collapsed, expanding to what was said, what a reviewer expected, what happened, and what to change. Failing conversations start open. Run both halves with `make eval-report`. |
...
| `rename_fork.py` | The mechanical half of adopting this repo as a base: rewrites the package name (which is also the console-script name), the `CONTACT_` environment prefix, the distribution and resource id, and the Terraform `name_prefix` default, in one pass. It prints a plan and writes nothing without `--yes`. The decisions it cannot make for you are the checklist in `docs/ADOPTING.md`. |
