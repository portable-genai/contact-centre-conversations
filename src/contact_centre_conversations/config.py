...
#: The port whose ACTIVE binding decides the model the pill names before any answer.
...
_GENERATOR_MODEL_ATTR: str = "model"
...
#: unrecognised value refuses at boot. See the fleet's runtime-control contract. The in-process
#: ``pii-kit`` masking in ``domain/pii.py`` is not the request-path redaction control the
#: contract switches, so it has no switch.
GUARDRAIL_ENV = "CONTACT_GUARDRAIL"
REVIEW_ROUTING_ENV = "CONTACT_REVIEW_ROUTING"
...
class Settings:
    ...
    def runtime(self) -> str:
        """WHERE this process runs, as the console's model pill states it: ``gcp`` or ``local``.
    ...
    def generator_model(self) -> str:
        """WHICH model answers, as the console's model pill first states it.
...
def _refuse_unconfigured_controls(settings: Settings) -> None:
    ...
    if controls.review_routing and not settings.review_url.strip():
        raise ConfiguredEmptyError(
    ...
    if controls.guardrail and not settings.guardrail_url.strip():
        raise ConfiguredEmptyError(
