...
os.environ.setdefault("CONTACT_PROFILE", "local")

#: Name BOTH MODES for the session, for the same reason and in the same way.
#:
#: Both gates ship OFF (see ``domain/modes.py``), so a suite that inherited the shipped default
#: would exercise no mode route at all and every one of them would answer 503. Turning them on
#: here is a deliberate statement, exactly like naming the profile above: nothing in the code
#: turns a mode on because nobody spoke.
#:
#: The tests that prove the OFF behaviour say so explicitly, by constructing settings with
...
os.environ.setdefault("CONTACT_AGENT_ASSIST_BUNDLE", "contact-centre-conversations-agent-assist")
os.environ.setdefault("CONTACT_SELF_SERVICE_BUNDLE", "contact-centre-conversations-self-service")
