...
from contact_centre_conversations.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from contact_centre_conversations.config import DEFAULT_BINDINGS, Settings
from contact_centre_conversations.ports.identity import (
    VERIFIED,
...
_API_MODULE = "contact_centre_conversations.api.app"
...
#: One canonical turn body for the end-user route these tests probe. The route only has to be a
#: real one that needs a principal; what it decides is another suite's subject.
_TURN_BODY: dict[str, object] = {
    "contact_id": "iap-probe",
    "market": "SG",
    "locale": "en-SG",
...
#: Every DATA port rebound to the SDK-free family under `gcp`, so this suite exercises the real
#: managed IDENTITY binding without a cloud project. Generated from ``DEFAULT_BINDINGS`` rather
#: than written out: the settings loader refuses a partial ``adapters:`` block, so a port added
#: in a later slice would silently make every test here fail to construct, and a hand-written
#: list is the thing that would rot.
_REBOUND_SETTINGS = "\n".join(
    [
        'audit_path: ":memory:"',
        ...
        *[
            line
            for port, table in sorted(DEFAULT_BINDINGS.items())
            for line in (
                f"  {port}:",
                f"    local: {table['local']}",
                # The identity port keeps its MANAGED binding: it is the subject of this suite.
                f"    gcp: {table['gcp'] if port == 'identity' else table['local']}",
                f"    onprem: {table['onprem']}",
            )
        ],
    ]
)
...
def test_an_emptied_audience_refuses_at_load_rather_than_resolving_to_empty(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Any
) -> None:
    """An operator who EMPTIED the audience expressed an intent, and it names nothing.

    Unset is "never configured" and answers 503 per the test above. Emptied is a different act,
    and the settings loader refuses it where the documented default is still known, so the
    process crashloops in the deploy with the variable named instead of serving requests on a
...
def test_a_verified_assertion_is_served_so_the_refusals_above_mean_something(
    ...
    assert body["next_step"]["instruction"], "a verified caller got an empty panel"
    assert body["next_step"]["citations"], "even the positive control must be cited"
