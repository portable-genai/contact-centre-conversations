...
# provider, so it proves the residency, retention, image-digest, mode-evidence and rule-R1/R8
# refusals at plan time and still touches nothing. Deliberately OUTSIDE `make gate`, which must
# not require terraform to be installed. A real `terraform plan` needs a project and is an
# operator step, never a build step: see docs/runbook.md.
tf-check:
