...
Contact Centre AI (E1): Contact-centre conversational AI with two separately gated modes.
Rendered from `hex-service-template`, so it starts at reference parity rather than converging
toward it. Package `contact_centre_conversations`, environment prefix
`CONTACT`, region `asia-southeast1`.
...
- `src/contact_centre_conversations/domain/` is PURE stdlib. No web framework, no cloud SDK,
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `CONTACT_S2S_TOKEN` may never enter that decision: it authenticates a
  ...
  persona and a real whisper panel. `tests/unit/test_end_user_auth_posture.py` walks the
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `CONTACT_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `CONTACT_PROFILE` (mentioning it in a refusal message is wanted, reading
...
- **Three-state environment reads, everywhere, enforced, in BOTH languages.** UNSET,
  ...
  only parsed Python is how `env.UI_TENANT_ORIGINS || "*"` survived the entire gate. **A read
  ...
  than a second copy of one rule. `_s2s.py` still passes `require_token=` for a
  non-loopback sibling, which is the one part of the policy the commons does not default to.
  Empty is the goal state, not a disabled check: the scanner is proved against a fictional
  reader, so it can still fail with nothing registered.
