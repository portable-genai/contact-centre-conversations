...
from contact_centre_conversations.adapters.local.audit import (
    LocalAuditAdapter,
)
...
_ANCHOR_ENV = "CONTACT_AUDIT_ANCHOR"


def _local(**overrides: object) -> Settings:
    """Settings for the anchor suite: the shipped packs and both modes on, like every test."""
    from tests.conftest import local_settings

    return local_settings(**overrides)
...
def _record_three(settings: Settings) -> LocalAuditAdapter:
    ...
    for submission in (
        sample_cases.OPENING_TURN,
        sample_cases.PII_TURN,
        sample_cases.OPENING_TURN,
    ):
        services.agent_assist.observe(submission, actor=sample_cases.ACTOR, as_of=utcnow())
    return audit
    ...
    with pytest.raises(AuditChainError):
        audit.record(
            AuditEvent(
                ...
                redacted_summary="one more ordinary append",
                mode="agent_assist",
            )
        )
