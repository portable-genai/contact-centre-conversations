"""FastAPI application for Contact Centre AI (E1).
...
``CONTACT_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``CONTACT_S2S_TOKEN`` is wrong in the most dangerous direction:
...
header and no ``X-Dev-Persona`` then got the seeded persona list and a real whisper panel.
...
from ..domain.models import ContactChannel, ContactRef, TurnSubmission
from ..domain.modes import ContactMode, ModeDisabledError
from ..ports.identity import VERIFIED, EndUserAuthUnavailableError
from ..services import ModeServices, build_services, require_mode
from .schemas import (
    AssistResponse,
    HealthResponse,
    ModeStatus,
...
#: The mode gates, resolved at IMPORT through ``Settings.load``. An empty or unrecognised mode
#: flag, or a mode enabled with no promotion evidence under a managed profile, therefore kills
#: the process here rather than at the first request. A serving process on a mode posture nobody
#: chose is the failure this line exists to prevent.
_MODE_GATES = Settings.load().modes
...
_CORS_ORIGINS_ENV = "CONTACT_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "CONTACT_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "CONTACT_S2S_AUDIENCE"
_API_HOST_ENV = "CONTACT_API_HOST"
...
def _submission(request: TurnRequest, *, mode: ContactMode, principal: Principal) -> TurnSubmission:
    """Build the domain turn from the request body and the VERIFIED principal.
    ...
    return TurnSubmission(
        ...
        role=ChannelRole(request.role),
        text=request.text,
        start_ms=request.start_ms,
        end_ms=request.end_ms,
        ends_contact=request.ends_contact,
    )
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``CONTACT_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``CONTACT_PROFILE`` must never be allowed to
    ...
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "CONTACT_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "contact-centre-conversations"
...
#
# Note what is NOT in this expression: CONTACT_S2S_TOKEN. A service
...
@app.post("/v1/agent-assist/turn", response_model=AssistResponse, tags=["agent-assist"])
def agent_assist_turn(
    request: TurnRequest,
    principal: Annotated[Principal, Depends(get_principal)],
) -> AssistResponse:
    """Produce the whisper panel for one turn of a live, human-handled contact.
...
@app.post("/v1/self-service/turn", response_model=SelfServiceResponse, tags=["self-service"])
def self_service_turn(
    request: TurnRequest,
    principal: Annotated[Principal, Depends(get_principal)],
) -> SelfServiceResponse:
...
def _run[T](work: Callable[[], T], *, mode: ContactMode) -> T:
    """Check the mode gate, then run, translating the two domain refusals into their statuses.

    The gate check happens BEFORE any work, so a disabled mode never touches a store, a model or
    a knowledge base. ``TenantMismatchError`` becomes 403 rather than 404 for the reason written
    in ``ports/contact_store.py``: a contact id is not a secret here, so hiding existence costs
    an operator the difference between "not yours" and "lost".
    """
    try:
        require_mode(_container(), mode)
...
def healthz() -> HealthResponse:
    """Liveness plus the mode posture, so an operator can see which modes this deployment serves.

    Reporting the gates here is deliberate: "the service is up but every mode route answers 503"
    is the single most confusing state this design can be in, and it is answered by one GET.
    """
    settings = _container().settings
    gates = (settings.modes.agent_assist, settings.modes.self_service)
    return HealthResponse(
        ...
        modes=[
            ModeStatus(
                ...
                promotion_bundle=gate.promotion_bundle,
            )
            for gate in gates
        ],
