...
`POST /v1/agent-assist/turn` answers 501 with the message below instead of a 500 with no body. Until it is
...
## Steps
1. Set `CONTACT_PROFILE=onprem`. A primary path that needs an unbound port
