...
from contact_centre_conversations.adapters._review_payload import (
    result_to_review,
)
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    ...
    result = services.agent_assist.observe(
        sample_cases.PII_TURN, actor=sample_cases.ACTOR, as_of=utcnow()
    )
    payload = repr(
    ...
    assert sample_cases.PLANTED_NRIC not in payload, (
        "a planted identifier reached the review payload: redaction is not merely late here, "
        "it is absent, because the turn was stored and cited before it was masked"
    )
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    local = build_services(build_container(local_settings()))
    panel = local.agent_assist.observe(
        sample_cases.OPENING_TURN, actor=sample_cases.ACTOR, as_of=utcnow()
    )
    assert panel.next_step.instruction, "an offline answer must still produce a next step"
    assert panel.next_step.citations, "an offline answer must still be cited"

    onprem = build_services(build_container(local_settings(profile="onprem")))
    with pytest.raises(NotImplementedError):
        onprem.agent_assist.observe(
            sample_cases.OPENING_TURN, actor=sample_cases.ACTOR, as_of=utcnow()
        )
